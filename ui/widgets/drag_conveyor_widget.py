...
class DragConveyorWidget(BaseEquipmentWidget):
    ...
    def build_input_panel(self):
        ...
        self.i_shaft_d  = InputGroup("샤프트 직경",       "mm",     20,  300,  60,  0,
                                      "축경 적정 여부 판단에 사용")
        self.i_eta      = InputGroup("전동 효율 η",       "",       0.5, 1.0,  0.90, 2)
        self.i_sf       = InputGroup("안전계수",          "",       1.0, 3.0,  1.25, 2)
        for w in [self.i_capacity, self.i_length, self.i_height, self.i_outlets,
                  self.i_F, self.i_E_mech, self.i_chain_v,
                  self.i_trough_w, self.i_trough_h, self.i_fill, self.i_density,
                  self.i_shaft_d, self.i_eta, self.i_sf]:
            l1.addRow(w)
    ...
    def collect_inputs(self):
        ...
        b = BearingInput(
            ...
            shaft_speed_rpm=16,   # 임시; 계산 중 sprocket_rpm으로 대체됨
