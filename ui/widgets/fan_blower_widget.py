...
class FanBlowerWidget(BaseEquipmentWidget):
    ...
    def build_input_panel(self):
        ...
        self.i_drv_eta  = InputGroup("전동 효율 η_d", "",      0.7, 1.0,    0.95, 2)
        self.i_sf       = InputGroup("안전계수",     "",       1.0, 3.0,    1.15, 2)
        for w in [self.i_flow, self.i_pressure, self.i_density,
                  self.i_fan_eta, self.i_drv_eta, self.i_sf]:
            l1.addRow(w)
        ...
        self.i_b_radial = InputGroup("반경 하중 Fr", "N",     100, 500000, 4000)
        self.i_b_axial  = InputGroup("축 하중 Fa",  "N",     0,   200000, 500)
        self.i_b_life   = InputGroup("요구 수명",   "hr",   1000, 100000, 40000)
        ...
        for w in [self.i_fan_rpm, self.i_b_radial, self.i_b_axial,
                  self.i_b_life, self.i_b_type, self.i_b_rel]:
            l2.addRow(w)
        self._input_layout.addWidget(g2)

        g3 = QGroupBox("샤프트 설계")
        l3 = QFormLayout(g3)
        self.i_s_bending  = InputGroup("굽힘 모멘트 M", "N·m", 0, 50000, 40)
    ...
    def collect_inputs(self):
        eq = FanBlowerInput(
            ...
            drive_efficiency=self.i_drv_eta.value(),
            safety_factor=self.i_sf.value(),
        )
        fan_rpm = self.i_fan_rpm.value()
        b = BearingInput(
