...
| 벨트 컨베이어 | 핸드북 Ch.1 | P = P₁+P₂+P₃, Pm = P/η |
| 플로우 컨베이어 | 핸드북 Ch.3 | H[HP] = E×L×Qt / 367 |
| 드래그 컨베이어 | 핸드북 Ch.4 | H[HP] = Qt×F×L×(1.2+0.3N) / (300×E) |

### 분리·집진류
| 장비 | 공식 출처 | 핵심 공식 |
|---|---|---|
| 백 필터 | 핸드북 Ch.8/9 | A = Qa/V, N = Qa/(V·π·D·H) |
| 사이클론 | 핸드북 Ch.10 | A = Qa/(Va·60), ΔP = λ·Va²/(2g)·γ |
| 로터리 밸브 | 핸드북 Ch.11 | Q = 0.7·W·(1-X)·N·60·γ |
| 체 (Sieve) | 핸드북 Ch.14 | Q = (k·l·m·n·o·p)·ρ'·a·q |
...
- [x] PDF / Excel 보고서 출력
- [x] 2026.05 핸드북 기반 장비 7종 추가 (벨트·플로우·드래그 컨베이어, 백필터, 사이클론, 로터리 밸브, 체)
- [ ] 계산 결과 JSON 저장/불러오기
